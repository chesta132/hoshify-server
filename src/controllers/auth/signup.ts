...
export const signup = async (req: Request, { res }: Response, next: NextFunction) => {
  try {
    ...
    const potentialUser = await User.findMany({ where: { OR: [{ email }, { gmail: email }] } }, { error: null });
    ...
    res.body({ success: createdUser }).sendCookie({ template: "REFRESH_ACCESS", rememberMe }).created();
  } catch (error) {
    next(error);
  }
};
